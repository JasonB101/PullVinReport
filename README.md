...
- Cached once per `cutout-v3|year|make|model|trim|color|body|engine` on the
  ...
  Keys that do not start with `cutout-v3|` are dropped on the next report view
...
1. Drop the old drawings so the next view cannot serve the white studio car.
   ...
   `cutout-v3|`, so a missed delete still will not show the old white image.
